...
class LibrispeechDataset(BaseDataset):
    def __init__(self, part, data_dir=None, index_dir=None, *args, **kwargs):
        assert part in URL_LINKS or part in {"train_all", "train_clean"}
        if data_dir is None:
        ...
        if part == "train_all":
            index_lsts = [self._get_or_load_index(part) for part in URL_LINKS if "train" in part]
            index = []
            for index_lst in index_lsts:
                index += index_lst
        elif part == "train_clean":
            index_lsts = [self._get_or_load_index(part) for part in ["train-clean-100", "train-clean-360"]]
            index = []
            for index_lst in index_lsts:
                index += index_lst
        else:
